        ...
        for (int axis = 0; axis < 3; axis++) {
            ...
            for (NodeIndex i = node.firstPrimitiveIndex();
                ...
                NodeIndex binIdx = clamp(
                    ...
                    0,
                    binCount - 1);
                // logger(EInfo, "getCentroid %d %d %d %d %d %d %d %d", binIdx,
                // getCentroid(m_primitiveIndices[i])[axis], boundsMin,
                // stepSize, node.aabb.min()[axis], node.aabb.max()[axis],
                // getBoundingBox(m_primitiveIndices[i]).min()[axis],
                // getBoundingBox(m_primitiveIndices[i]).max()[axis]);
