...
public:
    ...
    inline Color getColorAt(const Point2i &uv) const {
        ...
        case BorderMode::Clamp: {
            u = clamp(uv[0], 0, m_image->resolution()[0] - 1);
            v = clamp(uv[1], 0, m_image->resolution()[1] - 1);
            break;
        }
    ...
    Color evaluate(const Point2 &uv) const override {
        const float u = uv[0] * m_image->resolution()[0] - 0.5;
        const float v = uv[1] * m_image->resolution()[1] - 0.5;
